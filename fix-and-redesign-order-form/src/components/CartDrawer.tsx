    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="animate-drawer-in relative flex max-h-[88vh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl">
        ...
        <div className="border-t border-slate-100 bg-white px-6 py-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] sm:pb-5">
              ...
              {DELIVERY_OPTIONS.map((opt, i) => (
                <button
                  ...
                  className={`rounded-xl border px-3 py-2 text-left text-xs font-semibold transition ${
                    i === 0 || (i === DELIVERY_OPTIONS.length - 1 && (DELIVERY_OPTIONS.length - 1) % 2 === 1)
                      ? "col-span-2"
                      : ""
                  } ${i === 0 ? "border-2" : ""} ${
                    deliveryIdx === i
          ...
          {lines.length > 0 && subtotal < MIN_ORDER_VALUE && (
            <div className="mb-4 flex items-start gap-2 rounded-xl border border-orange-brand/40 bg-orange-brand/10 px-3 py-2.5 text-xs font-semibold leading-snug text-orange-brand">
              <span className="mt-0.5">⚠️</span>
              <span>
                A minimum order value of {formatR(MIN_ORDER_VALUE)} applies.
                Add {formatR(MIN_ORDER_VALUE - subtotal)} more to meet the
                minimum — orders below this amount will not be considered.
              </span>
            </div>
          )}
          ...
          <p className="mt-2.5 text-center text-[11px] leading-relaxed text-slate-400">
            WhatsApp opens with your order pre-typed - just tap Send. Please
            review your order carefully — orders cannot be returned once
            processed.
          </p>
        </div>
      </div>
