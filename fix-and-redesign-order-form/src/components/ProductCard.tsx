...
export default function ProductCard({ product, accent, qty, onAdd, onInc, onDec }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* thumbnail */}
      <div className="mb-3 flex h-28 items-center justify-center overflow-hidden rounded-xl bg-slate-50 pt-1">
        {showImage ? (
          <img
            src={product.image}
            alt={product.name}
          ...
          <span className={`flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br ${accent} text-2xl opacity-80`}>
            📦
          </span>
        )}
      </div>
